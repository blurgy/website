...
function MorphologyTable(word) {
	...
	// ( v-- under construction)
			...
			for (let m = 0; m < moods.length; m++) {
				...
				table += '<tr>';
				for (let n = 0; n < numbers.length; n++) for (let p = 0; p < persons.length; p++) table += `<th style="background-color: ${backgroundColor};">` + persons[p] + '</th>';
				table += '</tr>';
						...
						for (let n = 0; n < numbers.length; n++) {
							for (let p = 0; p < persons.length; p++) {
								...
								let props = [moods[m], voices[v], propertyTense, numbers[n], persons[p]],
								    ss = specialSyntax(word, props);
								// if special syntax
								if (ss) table += `<td style="text-align: center; ${word.type == 'verb Irr' ? '' : 'color: red;'} background-color: white;">` + ss + '</td>';
								// if no special syntax
								else table += '<td style="text-align: center; background-color: white;">' + conjugation[moods[m]][voices[v]][propertyTense][numbers[n]][persons[p]] + '</td>';
							}
						}
						table += '</tr>';
					}
